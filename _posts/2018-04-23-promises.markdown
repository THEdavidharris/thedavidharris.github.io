...
However, it should be pretty quickly that this approach has some serious shortcomings. The `completionHandler` takes in all optional parameters, some representing completely invalid states, like getting both data and an error, or all three as nil values. Using this completion handler requires handling all the valid states, as well as Swift-ly unwrapping the optional values, which isn't the cleanest way of extracting the call result.
